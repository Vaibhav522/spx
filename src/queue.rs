...
impl<S: JobSource> JobQueue<S> {
    pub async fn initalize(queue_capacity: usize, job_source: Arc<S>, max_job_fetch_attempt_count: usize, nats_context: async_nats::jetstream::context::Context) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let mut stream_context = nats_context.get_or_create_stream(stream::Config {
        ...
        let consumer_context = stream_context.get_or_create_consumer(&designated_consumer_name, pull::Config {
            ..Default::default()
        }).await?;

        // message stream context
        let message_stream_context = consumer_context.stream().max_messages_per_batch(queue_capacity).messages().await?;
    ...
    pub async fn add_jobs(&self, fetched_jobs: Vec<Vec<u8>>)  {
        for fetched_job in fetched_jobs {
            let _ = self.nats_context.publish(self.designated_stream_name.clone(), fetched_job.into()).await;
        }
    }
    ...
    pub async fn get_job(&mut self) -> Result<Job, Box<dyn std::error::Error>> {
        let fill_pct = (self.pending_queued_jobs.load(Ordering::Acquire) as f64 / self.queue_capacity as f64) * 100.0;

        if fill_pct > 30.0 && !self.requested_job_pull.load(Ordering::Acquire) && self.failed_attempt.load(Ordering::Acquire) < self.max_job_fetch_attempt_count {
...
impl JobUpdate {
    pub fn new(file_sha: String, output_file_path: Option<String>, is_transcoded: bool, processed_at: DateTime<Utc>, file_size: Option<usize>, time_to_process: std::time::Duration, error_faced: Option<String>) -> Self {
        Self {
            file_sha: file_sha,
...
impl<S: SyncSource> SyncUpdate<S> {
    pub async fn initalize(sync_threshold: usize, sync_source: Arc<S>, nats_context: async_nats::jetstream::context::Context) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        // designated name of job stream
    ...
    pub async fn sync_updates(&mut self, finished_queue: Option<bool>) -> Result<(), Box<dyn std::error::Error>> {
        if self.pending_queued_jobs.load(Ordering::Acquire) >= self.sync_threshold && !self.requested_db_sync.load(Ordering::Acquire) || finished_queue.unwrap_or(false) {
            ...
            let sync = self.sync_source.sync_updates(sync_jobs).await?;

            for row in rows {
                row.ack().await.map_err(|e| Box::new(e) as Box<dyn std::error::Error>)?;
            }
    ...
    pub async fn enqueue_update(&self, payload: JobUpdate) -> Result<(), Box<dyn std::error::Error>> {
        let byte_payload = serde_json::to_vec::<JobUpdate>(&payload)?;
        self.nats_context.publish(self.designated_stream_name.clone(), byte_payload.into()).await;
